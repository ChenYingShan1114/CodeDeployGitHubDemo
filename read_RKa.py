...
for i in range(len(name_list)):
    ...
    x1 = np.array(x1_list)
    y1 = np.array(y1_list)
    ax1.plot(x1[::3],y1[::3],'-o',linewidth=1,markersize=2,label=name_list[i]+" $\mathrm{fm}$")
...
ax1.set_xlabel("x")
ax1.set_ylabel("y")
ax1.set_title("Rutherford scattering simulated by adaptive Runge-Kutta method \n under different impact parameters")
ax1.set_ylim(-0.2e-12,0.2e-12)
ax1.set_xlim(-0.15e-12,0.15e-12)
ax1.legend()
ax1.grid()
plt.savefig('Rutherford_scattering_RKa.png')
