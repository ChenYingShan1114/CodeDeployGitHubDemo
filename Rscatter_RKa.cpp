...
int main(){
    ...
    int steps = 200;
    double dt = 3000;//total_time / steps;
    cout << dt << endl;
    double vx[steps+1], vy[steps+1], x[steps+1], y[steps+1], t_now[steps+1];
    vx[0] = v_in; vy[0] = 0; x[0] = ini_x; y[0] = b1, t_now[0] = 0;

    double vx_small_1 = 0, vy_small_1 = 0, x_small_1 = 0, y_small_1 = 0;
    double vx_small_2 = 0, vy_small_2 = 0, x_small_2 = 0, y_small_2 = 0;
    double vx_big     = 0, vy_big     = 0, x_big     = 0, y_big     = 0;
    double r_small = 0, r_big = 0;

    // dt越大、ideal_err越小，那回圈的迭代次數要越多。
    // steps越多，若要跑相同的距離，那ideal_err要小一點，dt才不會一下變太大。    
    for (int i = 1; i <= steps; i++){
        for (int j = 0; j <= 100; j++){ 
            // two small steps
            rk4(x[i-1], y[i-1], vx[i-1], vy[i-1], t_now[i-1], 0.5 * dt);
            vx_small_1 = vx_new;
            vy_small_1 = vy_new;
             x_small_1 = x_new;
             y_small_1 = y_new;
            rk4(x_small_1, y_small_1, vx_small_1, vy_small_1, t_now[i-1] + 0.5 * dt, 0.5 * dt);
            vx_small_2 = vx_new;
            vy_small_2 = vy_new;
             x_small_2 = x_new;
             y_small_2 = y_new;
             r_small = sqrt(x_small_2 * x_small_2 + y_small_2 * y_small_2);
            ...
            vy_big = vy_new;
             x_big = x_new;
             y_big = y_new;
             r_big = sqrt(x_big * x_big + y_big * y_big);
            
            double s1 = 0.99, s2 = 1.5;
            double ideal_err = 1e-10;
            // 若local_err > ideal_err大，dt就變小。
            // 若local_err < ideal_err小，dt就變大
            double local_err = fabs((r_small - r_big) / r_big);

            double dt_est = dt * pow(ideal_err / local_err, 0.2);
            double dt_new = s1 * dt_est;
            if (s1 * dt_est > s2 * dt){
                dt_new = s2 * dt;
            }
            else if (s1 * dt_est < dt / s2){
                dt_new = dt / s2;
            }
            else{
                dt_new = s1 * dt_est;
